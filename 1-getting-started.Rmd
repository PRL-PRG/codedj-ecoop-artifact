...
- **Skipping downloading**---how to use a pre-downloaded dataset without downloading anything,
...
Djanco and Parasite are already isntalled in the artifact image but you can 
download and build them Parasite using the following commands:
...
Next step is to download a dataset, but we also provide a pre-downloaded dataset in 
`datasets/predownloaded-toy-dataset`, so the reader may skip downloading the dataset.

To skip downloading, link the existing dataset to the path expected by the rest of the 
tools:

```{bash}
cd datasets
ln -s predownloaded-toy-dataset toy-dataset
cd ..
```
...
Optionally generate a cargo crate from a template we provide (remove pre-existing crate):
...
`query-results/getting-started/top_1_project_by_stars_in_each_language.csv` [truncated]:
...
Note that the values of attributes will have changed over time and the downloader will
